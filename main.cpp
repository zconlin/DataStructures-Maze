...
int main() {
	...
	std::cout << "end" << std::endl << std::endl; // indicate that the program has successfuly executed all instructions

	std::cout << "This program creates an algorithm to solve various 3D mazes. 5 mazes were provided by the instructor, with '1's representing open paths and '0's representing closed spaces (i.e. walls). Each maze is 5x5x5 (the top layer represented by the first 5x5 grid, the second layer by the next 5x5 grid, and so on), with the start in the top left of the top layer, and the exit at the bottom right of the bottom layer. The path can go left, right, up, down, and vertical up or down between layers, but not diagonally. Success is determined if outfile.txt matches with the associated keyfile.txt.";
	return 0;
}
